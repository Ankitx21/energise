...
def calculate_hour_angle(solar_time): return 15 * (solar_time - 12)
...
# ----- Statistics (clip negatives only) -----
def add_statistics_columns(df):
    model_columns = [c for c in df.columns if c.lower().startswith('model_') and c.lower().endswith('_pred')]
...
def calculate_energy_and_plot(df, date_str):
    ...
    # ----- PLOT FULL 24 HOURS -----
    plot_df = energy_df[:-1].copy()                  # exclude totals row
    ...
    ax.set_title('24-Hour Solar Irradiance Forecast with Prediction Bands')
    ax.legend()
...
# ----- Streamlit UI -----
def main():
    st.title("Solar Irradiance Forecast (24 h)")
    st.write("Enter coordinates to get a **full-day** irradiance forecast.")
    ...
    if st.button("Generate Forecast"):
        try:
            ...
            st.error("Please enter numeric coordinates.")
            return

        load_models()
        if not models:
            st.error("No models loaded. Place .pkl files in '34_models_all'.")
            return

        # Forecast starts at 00:00 of **tomorrow**
        forecast_time = (datetime.now().date() + timedelta(days=1))
        forecast_time = datetime(forecast_time.year, forecast_time.month,
                                 forecast_time.day, 0, 0)
        date_str = forecast_time.strftime("%Y-%m-%d")

        with st.spinner("Generating 24-h forecast..."):
            ...
            st.download_button(
                ...
                file_name=f"energy_values_{date_str}_24h.csv",
                mime="text/csv"
            )
